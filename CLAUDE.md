...
(+ since 2026-08-05 `duration_sec` — total review-audio seconds, ffprobe-measured at
export from the local masters, cached in `Trello/.audio_duration_cache.json`; null for
R2-only trips, which the backend measures itself into the `trip_durations` table; the
export also writes **`trip_durations.json`** at this repo's root — dave's local summary).
**Ordering/prioritisation (2026-08-05):** the admin can give any trip a numeric
**priority score** (`POST /api/trips/{id}/priority`, `trip_priority.score`; the FE
"Priority…" button) — scored trips order FIRST (highest score first) in every reviewer's
list, then pins (newest first), then Trello card order (`sessions._apply_priority_order`;
...
- **"Generate from edit" is GONE (2026-08-05, dave):** the full-field text-diff regen
  (`mode="segment"`) produced unreliable one-word candidates that combined at the wrong
...
- **Selection tools work for CJK too** (same isolation): highlight / alt-text / trim-noise /
  ...
  (trim/pause) never fall back silently — they 409. **Insert/Remove pause** (1s
...
- **Gate 2 (LLM judgment, cron `scripts/claude_review.py` → `auto_reviews`)** — Sonnet judges
  ...
  **Don't add it back.** Suggested zh fixes are `hsk_lib`-verified; nothing is ever auto-applied.
  ⚠️ **Gate 2 shells out to the `claude` CLI, whose login can lapse** — when it does, every
  run errors and is silently retried 6-hourly (Tokyo_03_Beg_N4_JP sat unreviewed 2 days,
  2026-08-04/05; fix = `claude` → `/login` on the laptop, interactive). `call_claude` now
  surfaces the real reason (it lives in the JSON envelope on stdout, not stderr) + the
  remedy. Redesign proposals (direct API call, failure alerting, JP-aware prompt + JP fix
  verification): `docs/auto-review-redesign.md`.
