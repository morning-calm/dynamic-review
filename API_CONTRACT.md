...
```jsonc
{
  ...
  "working_text": "…",                // what the WORKING take says (seeded to
                                      // original_text; re-set at each combine) — the
                                      // splice engine's diff baseline (_ZH sibling:
                                      // working_hans).
...
```jsonc
{
  ...
  "delta": null,                 // delta review only: { "created","reason","n_clips" } — the session
                                 // holds ONLY the changed clips of a COMPLETED trip (scenes[] is
                                 // trimmed to the changed scenes; trip_fields is empty). null otherwise.
...
| `GET /api/trips` | — | `[ { "trip_id","title","folder_name","lane","level","family","has_session","status","edit_required","reviewable","pinned","priority","language","duration_sec" } ]` — **filtered to the caller's language(s)** (admins see all; `language` is the trip's own suffix-derived language, for the admin's filter). `duration_sec` = total review-audio seconds (manifest-stamped by the Scripts export, else measured host-side and cached in `trip_durations`; `null` when unknown). **Excludes completed trips** (they move to `GET /api/completed`). **Ordered:** `priority`-**scored** trips first (highest score first), then admin-**pinned** (newest pin first), then Trello card order (the manifest's base sort). |
...
| `GET /api/deltas` | — | `[ { "trip_id","title","level","family","created","reason","scenes":[{"index","clips",…}],"n_clips","has_session","status" } ]` — **delta cards** (docs/delta-review.md): COMPLETED trips with an unconsumed `review-audio/_delta/<cid>.json` manifest (clips regenerated after approval). Language-filtered like `GET /api/trips`. A trip not in the completed queue (open full review / un-completed) never lists — the full review supersedes its delta. |
| `POST /api/deltas/{trip_id}/open` | — | `Session` — create/resume the trip's **delta session**: seeded from live staging text but holding ONLY the manifest's changed fields, audio pulled fresh from R2 `review-audio/<cid>/`. The trip **stays completed**. `403` wrong language; `404 no_delta` manifest gone (just consumed); `409 not_completed` if the trip is back in the full-review flow; `422 delta_audio_missing` when none of the manifest's clips are on R2. Review → submit → approve then run identically to a full session; **approve additionally deletes the `_delta/<cid>.json` manifest** (the consumed signal) and bumps `completed_at` (Stage-9 re-finalise). |
...
| `POST /api/sessions/{sid}/fields/{fid}/regenerate` | `{ "mode": "whole"\|"highlight"\|"alt", "range": {"start":int,"end":int}?, "alt_text": "…"? }` | `Field` with `audio.candidate` set **ASAP**. `highlight` re-voices the `range` selection; `alt` voices `alt_text` in place of the selection (`mode:"whole"` + `alt_text` voices it as the whole field); `whole` re-voices the whole field. (`"segment"` — "Generate from edit", the full-field text diff — was removed 2026-08-05 → `422`; an **English** SceneDesc `highlight`/`alt` without a `range` → `400 selection_required` — the CJK branch below has no such guard, a range-less `highlight` there whole-regenerates and a range-less `alt` voices the alt as the whole field.) **Selection reference text:** EN + `_JP` = `current_text` (the narration textarea; `_JP` selections must touch the KANA line → `409 kana_line_only` otherwise); `_ZH` = `localization.cur.Hans`. English engine: non-Latin / numeral-dense / Gemini-fallback → `flag:"edit_required"`, no candidate. **CJK (`_ZH` hanzi / `_JP` kana) SceneDesc:** `highlight` attempts a surgical char-level splice via the isolated forced aligner (highlight works with UNCHANGED text — the usual re-pronounce case); on any uncertainty `highlight` whole-regenerates and returns transient `"cjk_fallback": true` (UI hint: the whole clip changed), while `alt` sets `flag:"edit_required"` with no candidate (the alt is never voiced as the whole field or dropped). |
...
| `GET /api/review-queue` | — | `[ { "sid","trip_id","title","language","submitted_by","submitted_at","edit_required","delta" } ]` — **admin only.** Sessions awaiting approval. `delta:true` = a changed-clips re-review of a completed trip (approving consumes its delta manifest + re-finalises). |
...
| `DELETE /api/trips/{trip_id}/pin` | — | `{ "ok": true }` — **admin only.** Un-pin — the trip returns to the Trello base order (or its scored position, if it also has a priority). |
| `POST /api/trips/{trip_id}/priority` | `{ "score": number\|null }` | `{ "ok": true }` — **admin only.** Set a numeric priority (higher = review sooner; scored trips order above pins in every reviewer's list). `null` clears the score. |
...
| `POST /api/admin/pipeline/queue` | `{ "trip_id", "kind", "note"? }` | `BusJob` — **admin only.** Queue a job on the R2 review bus (`_bus/jobs/`). Kinds: `publish` (text, `publish_trip_text.py`) · `publish_docs` (single-trip doc release, `publish_trips_cli.py --trip` — first-of-family carries the TripGroup + rungs cascade) · `publish_pin` (staging pin → prod `CustomizableMenus`, `--pin`; **trip_id carries the TripLocation id**) · `add_to_location` (targeted prod `trips[]` insert, `--add-to-location`; **trip_id carries the TripGroup id**) · `thumbnail_local_copy` (R2 → the workstation's local thumbnails tree, replacing the family's existing jpg in place; **trip_id carries the TripGroup id**) · `replace_overlay` (R2 staged overlay → `stage10_static_check.py replace`; note carries `family=…;file=…`) · `publish_credits` (staging `CustomizableMenus/Credits` → prod via `publish_trips_cli.py --credits`) · `trello_move` (note carries `to=<lane>`; runs `Trello/trello_move.py`, native `--dry-run`) · `tool` (minted only by the publisher's `/pipeline/tool` endpoint, never queueable from a client). NOT best-effort: R2 failure → `502 bus_unavailable`; bad id → `422 bad_trip_id`. Nothing executes until a human runs it on the workstation. `BusJob = { id, kind, trip_id, note, requested_by, requested_at, status: "queued"\|"dry_run"\|"done"\|"failed", resolved_by?, resolved_at?, log? }`. |
...
| `POST /api/admin/pipeline/run` | `{ "job_id", "apply"?: bool, "i_am_sure"?: bool }` | `BusJob` — **admin only + PUBLISHER MODE only** (`403 not_publisher` unless the instance runs with `REVIEW_APP_PUBLISHER=1` — the workstation). Dispatches by the job's `kind` (see queue): shells `publish_trip_text.py` or `publish_trips_cli.py`, or runs the `thumbnail_local_copy` download inline (that one needs only `apply`, no `i_am_sure` — no prod write). Default DRY RUN; a real production write needs `apply` **and** `i_am_sure`, and still rides each script's own `--apply --i-am-sure` gates. Timeout/launch failure marks the job `failed` with the log. |
...
| `GET /api/tripdesc/categories?tg_id=…` | — | `{ "categories": [{name,count}], "scope" }` — **admin only.** The live category vocabulary with per-TripGroup counts; with `tg_id`, scoped to that group's COUNTRY via its TripLocations (global fallback when unmapped). |
| `GET /api/tripdesc/{tg_id}/category-check?category=…` | — | `CategoryCheck = { category, is_new, locations:[{name,country}], siblings:[{tg_id,has_category,mentions,snippet}] }` — **admin only.** Is the category new to the (country-scoped) vocabulary, and which same-location siblings' descriptions mention it without carrying it. Read-only. |
| `GET /api/final` | — | `{ "items": FinalCheckRow[], "audit": [{trip_id,method,completed_at,card_lane}], "manifest_has_final": bool }` — **admin only** (all `/api/final` routes are). The post-approval Final-check work list (docs/post-approval-admin-spec.md §2): one row per trip in Trello lanes 10/10b/11 (the manifest's `final_check` entries) + manual additions. `FinalCheckRow = { trip_id, lane: "10"\|"10b"\|"11"\|"manual", family, language, tg_id, tg_resolved, card_url, added_by, done, total, checks: {key: "open"\|"done"} }`. `audit` = completed trips on NO final-check card (with the card's current lane when known). |
| `GET /api/final/count` | — | `{ "open": n }` — nav badge: trips on the list with any check still open (0 for reviewers). |
| `POST /api/final/start` | `{ "trip_id" }` | `{ trip_id, started }` — manual "start final check" for a trip on no lane-10+ card (adds to the list; idempotent). |
| `GET /api/final/releases` | — | `{ groups: [{tg_id, in_prod, live_count, ready_count, locations: [{id, name, country}], jobs: [{id, kind, trip_id, status, note, requested_at}], rungs: [{trip_id, status: live\|re_review\|ready\|final_check\|reviewed\|in_review\|not_started, pending_delta, checks_done, checks_total, review_lane, completed_method, card_url, finalised, recall_quiz, four_k, keyword_copy}]}], prod_snapshot_at, prod_snapshot_has_rungs }` — the Publishing Queue board. `jobs` = recent bus jobs targeting the family (tg_id / rung cid / TripLocation doc id) for the inline chips. Readiness columns serve from the `release_probe_cache` materialisation (stale-served, background-refreshed, invalidated on job apply). Admin. |
| `GET /api/final/batches` | — | `{ batches: [{id, name, status: planned\|published\|archived, source: manual\|trello, trello_card, members: [{kind: trip\|group\|location, id}], created_by, created_at, updated_at, resolved: {trip_ids[], group_ids[], location_ids[]}, social: {state: ready\|partial\|missing\|unknown, meta, linkedin, news}}], social_probe }` — release batches (the Publishing Queue's group-by-batch unit + the social-drafting scope). `social` probes `Comms\Social Posts` — `unknown` on hosts without that tree, never a false `missing`. Admin. |
| `POST /api/final/batches` | `{ "name", "members", "id"?, "status"? }` | the saved batch (same shape) — create (name-upsert) or update. `422 bad_member`/`empty_name`/`bad_status`. |
| `POST /api/final/batches/import-trello` | — | `{ imported[], updated[], unmatched: [{card, token}] }` — seed/update batches from the Trello "TG Release Schedule" lane (one batch per card; structured family tables resolve to group members). Cards are a SEED — the app copy is the contract. `502 trello_unavailable/trello_error`. |
| `DELETE /api/final/batches/{id}` | — | `{ deleted }`. `404 no_batch`. |
| `GET /api/final/published?months=12` | — | `{ months, trips: [{trip_id, title, published_at, published_by, batch_id, source: publisher\|trello_backfill, note}] }` — the durable publish ledger (`published_trips`, stamped by the publish job's apply; history backfilled by `scripts/backfill_published_trips.py`). Drives "Recently published". Admin. |
| `GET /api/review-queue/count` | — | `{ "open": n }` — nav badge: submitted sessions awaiting the admin (0 for reviewers). |
| `GET /api/final/{trip_id}` | — | `{ trip_id, tg_id, tg_resolved, tg_exists, language, locations:[{name,country}], checks: FinalCheck[], description:{home,target,tripdesc_status}, categories[], title_key:{staging, prod_group, prod_trip, snapshot_at} }` — the per-trip checklist. `FinalCheck = { key, scope: "trip"\|"group"\|"location", scope_id, label, tooling, state, by, at, note }`; group/location-scoped checks are SHARED across siblings (stored per scope_id). `prod_group` needs a workstation prod snapshot taken after `publish_inbox.py` gained `trip_group` (2026-08-21); null = drift unknown. |
| `POST /api/final/{trip_id}/check/{check_key}` | `{ "state": "open"\|"done", "note"? }` | the updated `FinalCheck` — tick/untick a check at its scope. `422 bad_check_key` / `422 bad_state`. |
| `PUT /api/final/{trip_id}/title-key` | `{ "value" }` | `{ tg_id, contentTitleKey }` — targeted STAGING write of the TripGroup's `contentTitleKey`. `422 empty_title_key`; `409 tripgroup_missing`. |
| `PUT /api/final/{trip_id}/categories` | `{ "categories" }` | `{ tg_id, categories }` — targeted STAGING write of the TripGroup's `tripCategories`. `409 tripgroup_missing`. |
| `GET /api/final/{trip_id}/category-check?category=…` | — | `CategoryCheck` + `enrichment_matches: [{doc_id, tg_id, countries, hits:[{field,value}]}]` — the tripdesc sibling check PLUS ContentEnrichment country-mates whose signals (applicableCategories/newCategorySuggestions/themes/keyVocabulary/namedEntities) mention the category. Read-only. |
| `POST /api/final/{trip_id}/reopen-description` | — | `{ tg_id }` — check-1 escape hatch: ensures a tripdesc item exists for the family (seeding it if needed) and reopens it to `pending_en`; the FE navigates to `/descriptions/{tg_id}`. `409 tripgroup_missing`. |
| `GET /api/final/{trip_id}/location` | — | Check-4 read model: `{ trip_id, tg_id, locations: [{id, contentId, locationName, locationTitleKey, locationCountry, skyboxTextureId, trips[], groups:[{tg_id,exists,is_this_family}], pin:{menu_id,field,x,y}\|null}], menus: [{id, map_name, pins: MenuPin[], extra_buttons: MenuPin[]}], skyboxes: {used:[{id,count}], manifest: string[], manifest_generated_at} }`. `menus` = every staging `*_Trip_Menu`; `MenuPin = {LocationId,xPos,yPos}`. The FE renders `/maps/<MapName>.png` (committed from the Unity checkout; long axis = 18 units, origin top-left, +x right, −y down — MapPanelController). `manifest` comes from the committed `skybox_manifest.json` (`scripts/export_skybox_manifest.py`: S3 `360_Skyboxes/` ∪ the Unity tree). |
| `PUT /api/final/{trip_id}/location` | `{ "loc_id", "locationTitleKey"?, "skyboxTextureId"?, "trips"? }` | `{ loc_id, updated[] }` — targeted STAGING TripLocation update. `trips` must be a PERMUTATION of the live array (reorder only → `422 trips_not_permutation`); `409 location_missing` / `409 location_mismatch` (doc doesn't list the family). An unknown skybox id warns in the UI, never blocks. |
| `PUT /api/final/{trip_id}/pin` | `{ "loc_id", "menu_id", "x", "y" }` | `{ menu_id, field, loc_id, x, y }` — upsert the STAGING map pin (read-modify-update of `Pins`; an entry already in `ExtraMapButtons` updates there). Production is untouched — the Publisher's `publish_pin` job mirrors it at publish. `409 menu_missing`. |
| `GET /api/final/{trip_id}/thumbnail` | — | `{ tg_id, thumbnailTextureId, url, on_r2 }` — the family thumbnail: stem + public R2 url (`thumbs.dynamiclanguages.org/<stem>.jpg`, bucket ROOT — scene thumbs sit under `scene-thumbs/`); `on_r2` null = unverified (no creds). |
| `POST /api/final/{trip_id}/thumbnail` | multipart `file=<jpg>` | same + `local_copy_job` — replaces the thumbnail: R2 `<stem>.jpg` + staging `thumbnailTextureId` (existing stem kept; first upload mints `"<tg_id> - EN Trip app thumb"` per the local-tree convention), then best-effort queues a `thumbnail_local_copy` bus job for the workstation. `422 not_jpeg`/`too_large`; `502 r2_unavailable`. |
| `GET /api/final/{trip_id}/static-images` | — | Check-5 read model: `{ trip_id, scenes: [{scene_index, narration, audio_url, overlays: [{filename, appear, disappear, url}]}], rules: {min_appear,min_display,max_display,gap} }` — every scene carrying `staticImages[]`. `rules` are the stage9/stage10 house rules (warn-only). |
| `PUT /api/final/{trip_id}/static-images/timing` | `{ "scene_index", "filename", "appear", "disappear" }` | `{ …, warnings[] }` — targeted staging write of one overlay's appear/disappear (INTS; mirrors `stage10_static_check.py set-timing`: fresh read → whole-quickTrips update). Only `appear < disappear` hard-fails (`422 bad_timing`); the house rules come back as `warnings`. |
| `POST /api/final/{trip_id}/static-images/replace?filename=…` | multipart `file=<jpg/png>` | `{ filename, r2_key, family, replace_job }` — stages the new overlay to R2 `review-overlays/<base>/` (instant display) and queues a `replace_overlay` bus job: the workstation runs `stage10_static_check.py replace`, the CANONICAL distribution (country folders under `D:\Dynamic Languages` + Drive + OGG trees + S3). `404 no_overlay` when no scene lists the filename. |
| `GET /api/final/{trip_id}/scenes/{i}/audio?kind=narration\|question\|answer\|option&opt=n` | — | `audio/mpeg` (Range) from the trip's resolved master dir — `{i}.mp3` / `{i}_q.mp3` / `{i}_a.mp3` / `{i}_a{n}.mp3` (0-based scene index, sessions.mp3_name naming). Cookie-auth for `<audio>` elements. |
| `GET /api/final/{trip_id}/overlays/{filename}` | — | the overlay image: local trees first, else 307 → the public R2 `review-overlays/<base>/` copy. Cookie-auth. |
| `GET /api/final/credits` · `POST /api/final/credits` | POST: `{ "header", "entry" }` | `{ exists, credits: [{header, entries[]}] }` — the VR app's SINGLE `CustomizableMenus/Credits` doc (one credits button; format fixed by CreditsPanel.cs). ADD-ONLY: append an entry under a header (new header/doc created in the same shape); `409 duplicate_credit`. Reaches production via the `publish_credits` job (`publish_trips_cli.py --credits`, which refuses if prod holds blocks staging lacks). |
| `GET /api/final/{trip_id}/keywords` | — | Check-6 read model: `{ trip_id, language, scenes: [{scene_index, question, question_en, options[], correct, additional[], is_keyword, question_audio, answer_audio}] }` — every Q&A/keyword scene (`correct` = `questionOptionKeys[0]`, or `questionKey` for speak-and-repeat). |
| `POST /api/final/{trip_id}/answer-keys` | `{ "scene_index", "key" }` | `{ scene_index, additional[] }` — ADD-ONLY append to `additionalAnswerKeys`. `409 collides_with_option` when the key equals another (wrong) option (the stage9/answer_keys.py safety rule); `409 duplicate_key`. |
| `GET /api/final/speech-token` | — | `{ "token", "region" }` — short-lived Azure Speech token for the browser mic (10-min tokens, 8-min server cache). The subscription key stays server-side: `Azure_Key1` (or `AZURE_SPEECH_KEY`) + `AZURE_SPEECH_REGION` (default `uksouth`) in the Scripts `.env`. `503 azure_not_configured` until the key exists. |
| `POST /api/final/{trip_id}/ready` | — | `{ trip_id, publish_job, trello_job }` — ALL 7 checks green → queues the release: a `publish_docs` bus job + a `trello_move` job (`to=11`). `409 checks_open` (lists the open keys). The workstation Publisher executes both. |
| `POST /api/admin/pipeline/tool` | `{ "tool", "target"?, "steps"?, "lane"?, "apply"? }` | `BusJob` (kind `tool`) — **publisher mode only.** Runs one WHITELISTED Scripts tool; long runs finish on a background thread and stamp the job (watch the inbox). Tools: `bump_version` (BumpContentVersion --prod; apply-gated, no target) · `trello_move` (--to `lane`, native --dry-run) · `docids_append` (dated line under an auto section at the end of Content_DocIDs.md; apply-gated) · `snapshot` (publish_inbox.py snapshot) · `tripdocs` (tripdocs_local; apply = --upload) · `static_pic_4k` (build --cids; apply = --apply) · `upload_thumbs` (upload_thumbnails_r2; dry = --dry-run) · `stage10b` (--families; NEVER passes --apply-trello — the family card moves lanes only via mark_published when every rung is stamped) · `stage9_finalise` (needs `steps`; apply-gated, 90-min timeout) · `group_text_drift` (publish_trips_cli --sync-group-text; blank target = the ALL sweep; apply = --apply --i-am-sure). Write-tools with no native dry-run return a `dry_run` job describing the exact command instead of inventing one. `422 bad_tool/bad_target/bad_steps/bad_lane`. |
| `GET /api/admin/publisher-mode` | — | `{ "publisher_mode": bool }` — cheap nav probe: is THIS instance the workstation publisher. |
| `POST /api/admin/pipeline/gate-report` | — | `{ "ok", "log" }` — **publisher mode only.** Runs `publish_audio_gate.py --report` (read-only sweep, minutes). |
