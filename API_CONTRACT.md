...
| `GET /api/admin/staging-trips?q=…&refresh=1` | — | `{ "total", "shown", "trips": [ { "trip_id","title","folder_name","language","has_session","status","edit_required","completed_method","completed_by" } ] }` — **admin only.** Substring search over the WHOLE staging `Trips` collection (server-cached index, TTL 10 min; `refresh=1` forces a re-sweep). Hard-capped at 200 rows (`total` = matches before the cap). |
| `POST /api/admin/open` | `{ "trip_id": "…" }` | `Session` — **admin only.** Create/resume a session for ANY staging trip, **including a completed one** (the reviewer flow 409s there). Seeding still requires resolvable MP3 masters (`422 bad_folder`). |
...
| `POST /api/admin/structure/{trip_id}/add` | `{ "position", "base", "video_url"?, "is_static"?, "scene_id"? }` | Same shape — inserts an empty-text scene. sceneId = supplied existing atom / derived-from-stem / minted, always via the shared `scene_ids` ruleset (Scripts repo); registry `usedBy` gains the trip. |
| `POST /api/admin/structure/{trip_id}/swap-video` | `{ "index", "video_url", "rekey", "base", "scene_id"? }` | Same shape — `rekey:false` = same footage/new encode (sceneId KEPT; registry gains the videoId); `rekey:true` = genuinely different scene (new atom id; old registry use released; translations keyed to the old id orphan until re-authored + recompiled). |
